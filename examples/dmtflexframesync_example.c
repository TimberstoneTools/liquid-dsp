//
// dmtflexframesync_example.c
//
// Example demonstrating the DMT flexible frame synchronizer.
//
...
void usage()
{
    printf("dmtflexframesync_example [options]\n");
...
int main(int argc, char*argv[])
{
    ...
    unsigned char p[M];
    dmtframe_init_default_sctype(M, p);
    dmtframe_print_sctype(p, M);

    // create frame generator
    dmtflexframegenprops_s fgprops;
    dmtflexframegenprops_init_default(&fgprops);
    fgprops.check           = check;
    fgprops.fec0            = fec0;
    fgprops.fec1            = fec1;
    fgprops.mod_scheme      = ms;
    dmtflexframegen fg = dmtflexframegen_create(M, cp_len, taper_len, p, &fgprops);

    // create frame synchronizer
    dmtflexframesync fs = dmtflexframesync_create(M, cp_len, taper_len, p, callback, (void*)payload);
    if (debug_enabled)
        dmtflexframesync_debug_enable(fs);
    ...
    for (i=0; i<payload_len; i++)
        payload[i] = rand() & 0xff;
    dmtflexframegen_assemble(fg, header, payload, payload_len);
    dmtflexframegen_print(fg);
    dmtflexframesync_print(fs);

    // initialize frame synchronizer with noise
    for (i=0; i<1000; i++) {
        float complex noise = nstd*( randnf() + _Complex_I*randnf())*M_SQRT1_2;
        dmtflexframesync_execute(fs, &noise, 1);
    }
    ...
    while (!last_symbol) {
        // generate symbol
        last_symbol = dmtflexframegen_writesymbol(fg, buffer);
        ...
        // receive symbol
        dmtflexframesync_execute(fs, buffer, frame_len);
    }
    nco_crcf_destroy(nco);

    // export debugging file
    if (debug_enabled)
        dmtflexframesync_debug_print(fs, "dmtflexframesync_debug.m");

    // destroy objects
    dmtflexframegen_destroy(fg);
    dmtflexframesync_destroy(fs);
